...
public class JavaSampleServerApplication {
    ...
    @Bean
    RouterFunction<ServerResponse> route(WebClient client, @Value("${api.url}") String apiUrl) {
        return RouterFunctions.route()
                .GET("/sayhello", request ->
                        client.get().uri(URI.create(apiUrl).resolve("/hello")).retrieve().bodyToMono(String.class)
